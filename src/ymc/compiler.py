...
]  # python is weird and won't let you define a boolean array with 0 and 1 so we have to use true and false here
# ZF     SF     OF     CF
flags = [
    False, False, False, False
]  # maybe I'm wrong and one of you guys can figure out how to be able to use 0 and 1
...
def declaration():  # start by checking if signed or unsigned
    print(
        "This is case 1"
    )  # I left these print statments here to eliminate the error caused by having no code in the function
...
def relational(
):  # if/else and while statements, start by checking what each line is
...
switch_dict = {1: declaration, 2: arithmetic, 3: relational, 4: printD}
