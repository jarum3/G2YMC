...
variables_set: dict[str, bool] = {"a": False, "b": False, "c": False, "x": False, "y": False, "z": False}
variable_value: dict[str, int] = {"a": 0, "b": 0, "c": 0, "x": 0, "y": 0, "z": 0}
...
def declaration(pline_instance): # start by checking if signed or unsigned
    ...
    if line_text.startswith("unsigned"):  # start by checking if signed or unsigned
        ...
        for v in variables:               # go through each variable and assign to location in storage
            print(v)
    elif pline_instance.text.startswith("signed"):
